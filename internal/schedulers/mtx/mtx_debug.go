...
func Init(maxQueue int) {
	println("MTX - main thread scheduler initialized.")
	println("MTX debug tag enabled.")
...
func Run(f func()) {
	...
	go func() {
		println("mtx.goroutine() start")
		f()
		doneSignal <- struct{}{}
		println("mtx.goroutine() stop")
	}()

	handler()

	println("mtx.Run() finished!")
}
...
func handler() {
	...
	for running == true {
		...
		case mtxcall := <-queue:
			...
			currdec()
		case <-doneSignal:
			println("mtx.handler() received done signal, stopping...")
			running = false
		}
	}
	PrintStats()
